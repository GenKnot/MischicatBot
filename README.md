...
- ~~拆分 `cogs/public_events.py`（灵雨 views → `utils/views/spirit_rain.py`，万宝楼公共 views → `utils/views/wanbao_public.py`，cog 本体保留调度/命令逻辑）~~
- 拆分 `cogs/cultivation.py`（按职责拆分为多个子模块）
- 数据库层逐步迁移至 SQLAlchemy async（新功能用 `utils/db_async.py`，旧代码按模块逐步替换）
- 功法装备上限按境界解锁（炼气3本 · 筑基5本 · 结丹7本 · 元婴10本 · 化神15本）
- 化神期突破（需补全五行灵根）
- 锻造系统（矿石/木材实际用途，打造武器防具）
- 炼丹系统（草药配方，炼制丹药，炼丹师品级，丹纹品质）
